...
@Component({
  selector: 'app-bin-revenues',
  standalone: true,
  imports: [CommonModule, RouterModule, FormsModule, ReactiveFormsModule, SharedModule],
  templateUrl: './bin-revenues.component.html',
  styleUrls: ['./bin-revenues.component.scss']
})
export class BinRevenuesComponent implements OnInit {
  revenueData: any[] = [];
  isLoading = false;
  ...
  loadRevenueData(): void {
    ...
    this.epinService.getBinRevenues(params).subscribe({
      next: (response) => {
      ...
      error: (error) => {
        console.error('Error loading BIN revenues:', error);
        this.error = 'Failed to load BIN revenue data. Please try again.';
        this.isLoading = false;
      }
    });
  }
  ...
  exportData(format: 'csv' | 'excel'): void {
    ...
    this.epinService.exportReport(format, params).subscribe({
      next: (blob) => {
        ...
        a.href = url;
        a.download = `bin-revenues-${this.startDate}-to-${this.endDate}.${format}`;
