...
import { ServiceBreakdownResponseDto, VisaSumamryQueryParams } from '../../../../core/models/invoice.models';
   ...
   // Pagination properties
    ...
    loadInvoices(): void {
      ...
      const service = this.activeProvider === 'visa'
        ? this.billingService.getVisaServiceBreakdown(params)
        : this.billingService.getMastercardServiceBreakdown(params);
      ...
      service.subscribe({
        ...
        error: (error) => {
          console.error(`Error loading ${this.activeProvider.charAt(0).toUpperCase() + this.activeProvider.slice(1)} services breakdown:`, error);
    ...
    exportBreakdownToCsv(): void {
      console.log('Exporting Visa breakdown to CSV...');
      ...
      this.billingService.getVisaServiceBreakdown(params).subscribe({
        next: (data) => {
          ...
          if (
            ...
            this.downloadCsv(csv, 'visa-breakdown-report.csv');
          } else {
